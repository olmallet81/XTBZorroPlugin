...
{
	set(LOGFILE);
	if(To != TO_CSV) print(To,"\n");
	if(is(TRADEMODE))
		print(To,"Class,Type,Expiry,Strike,Underlying,Ask,Bid,Multiplier,Vol,Symbol");
	else
		print(To,"Date,Type,Expiry,Strike,Underlying,Ask,Bid,Val,Vol");
	int i = 0;
	if(Handle > 0) {
		CONTRACT* c;
